...
Decompress frame
*****************************************************************************************/
int CUnMAC::DecompressFrame(unsigned char *pOutputData, __int32 FrameIndex, int CPULoadBalancingFactor) 
{
...
*****************************************************************************************/
int CUnMAC::DecompressFrameOld(unsigned char *pOutputData, __int32 FrameIndex, int CPULoadBalancingFactor) 
{
	//error check the parameters (too high of a frame index, etc.)
	if (FrameIndex >= m_pAPEDecompress->GetInfo(APE_INFO_TOTAL_FRAMES)) { return 0; }
	...
	if (GET_USES_CRC(m_pAPEDecompress) == FALSE)
	{
		unsigned __int32 nChecksum = CalculateOldChecksum(m_pAPEDecompressCore->GetDataX(), m_pAPEDecompressCore->GetDataY(), m_pAPEDecompress->GetInfo(APE_INFO_CHANNELS), nBlocks);
...
*****************************************************************************************/
unsigned __int32 CUnMAC::CalculateOldChecksum(int *pDataX, int *pDataY, int nChannels, int nBlocks)
{
